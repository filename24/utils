...
import { BaseCommand, Command, IBaseCommand, SlashCommand } from '../structures/Command';
...
export class CommandManager extends BaseManager {
  ...
  public get(commandName: string): IBaseCommand | undefined {
    let command;
    if (this.client.commands.has(commandName)) return (command = this.client.commands.get(commandName));

    this.client.commands.forEach((cmd) => {
      if (this.isSlash(cmd) && cmd.data.name === commandName) return (command = cmd);
      // @ts-ignore
  ...
  public isSlash(command: IBaseCommand | undefined): command is SlashCommand {
  ...
  public async slashCommandSetup(guildID: string): Promise<ApplicationCommandDataResolvable[] | undefined> {
    ...
    this.client.commands.forEach((command: IBaseCommand) => {
      ...
      for (const command of slashCommands) {
        ...
        if (!cmd) {
          await this.client.application?.commands
            .create(command)
            .then((guilds) =>
              this.logger.info(`Succesfully created command ${command.name} at ${guilds.name}(${guilds.id}) guild`),
            );
        }
      }
    } else {
